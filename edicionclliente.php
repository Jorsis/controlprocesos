...
$consulta=$db->Execute("SELECT * FROM cliente");
...
function creaInput(idDiv, idInput, name)
{
	...
	if(!mostrandoInput && divError.style.display!="block")
	{
		...
		// Creamos el input
		divContenedor.innerHTML="<input type='text' onBlur='cargaDatos(\""+idDiv+"\", \""+idInput+"\", \""+name+"\")' value='"+divContenedor.innerHTML+"' id='"+idInput+"' maxlength='50'>";
	...
	if(!mostrandoInput && divError.style.display!="block")
	{
		...
		mostrandoInput=true;
			...
			?>
			  ...
			  <tr><td>
				<div id="demoArr<?= $consulta->fields[0]?>a" style="border:1px dashed; width:150px; background-color:#EAEAEA; text-align:center;" onclick="creaInput(this.id, '<?= $consulta->fields[0]?>','id_usuario')"><?=$consulta->fields[0];?></div>
			  </td>
			  <td>
				<div id="demoArr<?= $consulta->fields[0]?>b" style="border:1px dashed; width:150px; background-color:#EAEAEA; text-align:center;" onclick="creaInput(this.id, '<?= $consulta->fields[0]?>','nombre_cliente')"><?=$consulta->fields[5];?></div>
			  </td>
			  <td>
				<div id="demoArr<?= $consulta->fields[0]?>c" style="border:1px dashed; width:150px; background-color:#EAEAEA; text-align:center;" onclick="creaInput(this.id, '<?= $consulta->fields[0]?>','nombre_usuario')"><?=$consulta->fields[1];?></div>
			  </td>
			  <td>
			  ...
			  <td>
				<div id="demoArr<?= $consulta->fields[0]?>e" style="border:1px dashed; width:150px; background-color:#EAEAEA; text-align:center;" onclick="creaInput(this.id, '<?= $consulta->fields[0]?>','telefono')"><?=$consulta->fields[7];?></div>
			  </td>
			  <td>
				<div id="demoArr<?= $consulta->fields[0]?>f" style="border:1px dashed; width:150px; background-color:#EAEAEA; text-align:center;" onclick="creaInput(this.id, '<?= $consulta->fields[0]?>','direccion')"><?=$consulta->fields[6];?></div>
			  </td>
			  <td>
				<div id="demoArr<?= $consulta->fields[0]?>g" style="border:1px dashed; width:150px; background-color:#EAEAEA; text-align:center;" onclick="creaselect(this.id, '<?= $consulta->fields[0]?>','activo')"><?=$consulta->fields[8];?></div>
			  </td>
			  </tr>
