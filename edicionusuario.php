...
$consulta=$db->Execute("SELECT * FROM usuario_control order by nombre");
...
function cargaDatos(idDiv, idInput, name)
{
	...
	else
	{
		...
		ajax.open("GET", "guardarcambios.php?dato="+valorInput+"&codigo="+idInput+"&name="+name+"&user=usuario", true);
		ajax.send(null);
	...
	if(!mostrandoInput && divError.style.display!="block")
	{
		...
		mostrandoInput=true;
			...
			<div id="demo" style="width:500px;">
			    ...
			    <tr>
			      <td align="center"><h3>Cedula</h3></td>
			...
			?>
			  ...
			  <td>
				<div id="demoArr<?= $consulta->fields[0]?>b" style="border:1px dashed; width:150px; background-color:#EAEAEA; text-align:center;" onclick="creaInput(this.id, '<?= $consulta->fields[0]?>','nombre')"><?=$consulta->fields['nombre'];?></div>
			  </td>
			  <td>
